    ...
    >
      ...
      <div className="absolute top-4 right-4 z-10">
        <Badge
          ...
          severity={post.isPublished ? "success" : "warning"}
          className={`${
            post.isPublished
      ...
      <div className="p-6">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg">
                <Icon icon="mdi:note-text" className="w-6 h-6 text-white" />
              </div>
              {!post.isPublished && (
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
              ...
              Shared to Departments
            </h4>
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {post.postDepartments?.length ?? 0} departments
            </span>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            {post.postDepartments?.slice(0, 5).map((pd, index) => (
              <div key={pd.id} className="relative">
                <Avatar
                  label={pd.department?.departmentName?.charAt(0) ?? "?"}
                  shape="circle"
                ...
                />
                <Tooltip
                  target={`.post-collab-${pd.id}`}
                  content={pd.department?.departmentName ?? "Unknown"}
                  position="top"
                />
                <div
                  className={`post-collab-${pd.id} absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-white dark:border-gray-800 rounded-full`}
                />
              </div>
            ))}

            {post.postDepartments && post.postDepartments.length > 5 && (
              <Avatar
                label={`+${post.postDepartments.length - 5}`}
                shape="circle"
        ...
        <div className="flex items-center gap-3">
          ...
          <Tooltip
            target=".edit-post-button"
            content="Edit Post"
            position="top"
          />
          <Button
            className="edit-post-button w-12 h-12 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 rounded-xl transition-all duration-200 transform hover:scale-105 shadow-sm hover:shadow-md flex items-center justify-center"
            onClick={() => handleEdit()}
          >
